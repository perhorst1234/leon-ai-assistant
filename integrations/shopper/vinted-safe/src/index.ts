...
function money(x:any): {price:number|null,currency:string} { const n=typeof x?.price === "object" ? x.price.amount : x?.price; const p=Number(n); return {price:Number.isFinite(p)&&p>=0?p:null,currency:clean(typeof x?.price === "object" ? x.price.currency_code : (x?.currency ?? x?.currency_code),8)}; }
function items(data: any): any[] { return (Array.isArray(data?.items) ? data.items : []).slice(0,MAX_ITEMS).map((x:any)=>{const iid=itemId(x.id); const m=money(x); return {id:iid,title:clean(x.title),price:m.price,currency:m.currency,brand:clean(x.brand_title ?? x.brand,80),condition:clean(x.condition,40),size:clean(x.size,40),favouriteCount:finite(x.favourite_count ?? x.favouriteCount),url:iid?`https://www.vinted.nl/items/${iid}`:null};}); }
function prices(data: any): unknown { const vals = items(data).map((x:any)=>x.price).filter((x:any): x is number=>typeof x === "number"); if (!vals.length) return {count:0}; vals.sort((a,b)=>a-b); const mid=Math.floor(vals.length/2); return {count:vals.length,min:vals[0],max:vals[vals.length-1],average:Math.round(vals.reduce((a,b)=>a+b,0)/vals.length*100)/100,median:vals.length%2?vals[mid]:(vals[mid-1]+vals[mid])/2}; }
async function call(name: string, a: Record<string,unknown>): Promise<string> {
  switch(name) {
    ...
    case "get_item": { const raw:any=await get(`/api/v2/items/${id(a.itemId)}`); const x=raw?.item ?? raw; const iid=itemId(x?.id); const m=money(x); return envelope({item:{id:iid,title:clean(x?.title),description:clean(x?.description,2000),price:m.price,currency:m.currency,brand:clean(x?.brand_title ?? x?.brand,80),condition:clean(x?.condition,40),size:clean(x?.size,40),url:iid?`https://www.vinted.nl/items/${iid}`:null}}); }
    case "get_seller": { const raw:any=await get(`/api/v2/users/${id(a.sellerId)}`); const x=raw?.user ?? raw; const sellerId=itemId(x?.id); return envelope({seller:{id:sellerId,username:clean(x?.login ?? x?.username,80),feedback_rating:typeof x?.feedback_rating === "number" ? x.feedback_rating : clean(x?.feedback_rating,20),feedback_count:typeof x?.feedback_count === "number" ? x.feedback_count : clean(x?.feedback_count,20),city:clean(x?.city,80),member_since:clean(x?.created_at,40),url:sellerId?`https://www.vinted.nl/member/${sellerId}`:null}}); }
    ...
    case "get_trending": { const x:any=await get("/api/v2/catalog/items", {search_text:text(a.query) ?? "", order:"relevance", per_page:String(integer(a.limit,20,MAX_ITEMS))}); const ranked=items(x).sort((left,right)=>(right.favouriteCount ?? -1)-(left.favouriteCount ?? -1)); return envelope({query:clean(a.query),ranking:"favorite_count_within_sample",items:ranked}); }
