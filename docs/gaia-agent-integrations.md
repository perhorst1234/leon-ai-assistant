...
| Vinted search | local `vinted-safe-stdio` 1.0.0 derived from supplied 0.1.2 source | hash-locked non-executing staging installer ready; binding disabled | Rootless Podman protocol and egress attestation; then promote read-only manifest and binding. |
| AliExpress search | local `aliexpress-safe-stdio` 1.0.0 derived from supplied Fetchaller 3.5.4 source | four-tool sanitized fork and hash-locked non-executing staging installer ready; binding disabled | Rootless Podman protocol and exact `www.aliexpress.com` egress attestation; then promote read-only manifest and binding. |
