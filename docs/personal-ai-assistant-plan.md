...
- **Backend volledig lokaal teruggevonden en gedeployed:** Python-control-plane, privé-SQLite en webservices draaien op de doelserver. Chat en expliciet toegepaste text-only agentruns gebruiken de echte lokale Ollama/M40-route; OpenAI-adapter blijft dry-run. Zie [herstelbewijs](wsl-recovery.md) en [M40-bewijs](m40-deployment-evidence.md).
