...
test('model preview/submission and UUID recovery use only explicit local routes', async () => {
  for (const [resource, method, path] of [
    ...
    ['reconciliation&id=work-a', 'GET', '/api/work/model/reconciliation?id=work-a'],
    ['reconciliation', 'POST', '/api/work/model/reconciliation'],
  ]) {
  ...
  for (const resource of ['model-preview', 'model', 'reconciliation', 'reconciliation&id=', 'reconciliation&request_id=x', 'tasks&request_id=x', 'jobs&id=x&request_id=y', 'jobs&request_id=a&request_id=b']) {
    assert.equal((await forwardLeon(request(resource), config, forbiddenFetch)).status, 404);
  }
