...
import { Brain, Check, Database, Link2, LockKeyhole, Plus, RefreshCw, Search, Trash2, Unplug } from 'lucide-react';
...
export default function ConnectedMemory({ demo }: { demo: ReactNode }) {
  ...
  async function refresh(activeToken = token): Promise<string | null> {
    controller.current?.abort(); setSearching(false); const next = new AbortController(); controller.current = next;
    try { const data = await api(activeToken)('memory', undefined, next.signal); if (next.signal.aborted) return null; if (!Array.isArray(data.items)) throw new Error('Onverwacht geheugenantwoord.'); setItems(data.items); setSnapshotItems(data.items); setEdges(Array.isArray(data.graph_edges) ? data.graph_edges : []); setSelectedId(current => data.items!.some(item => item.id === current) ? current : data.items![0]?.id || ''); setQuery(''); setError(''); return null; }
  ...
  async function connect() { const nextToken = draftToken.trim(); setToken(nextToken); setDraftToken(''); setItems([]); setSnapshotItems([]); setEdges([]); setSelectedId(''); setNotice(''); setEditMode(null); await refresh(nextToken); }
  function disconnect() { controller.current?.abort(); setToken(''); setItems([]); setSnapshotItems([]); setEdges([]); setSelectedId(''); setError(''); setNotice(''); setEditMode(null); setSearching(false); }
  ...
  return <>
    <div className="work-mode-switch" aria-label="Geheugenmodus"><button type="button" aria-pressed={!showDemo} onClick={() => { setShowDemo(false); if (token) void refresh(); }}>Echt geheugen</button><button type="button" aria-pressed={showDemo} onClick={() => setShowDemo(true)}>Ontwerpvoorbeeld</button></div>
    {showDemo ? <><span className="work-demo-label">Voorbeelddata — geen echte wijzigingen</span>{demo}</> : <div className="space-view work-view connected-memory">
      <header className="work-header"><div><h1>Levend geheugen.</h1><p>Opgeslagen context met bron, vertrouwen en controle over wat Gaia mag onthouden.</p></div><div className="work-actions">{token && <><button className="secondary-control" type="button" onClick={() => void refresh()} disabled={busy}><RefreshCw size={15}/> Ververs</button><button className="secondary-control" type="button" onClick={disconnect} disabled={busy}><Unplug size={15}/> Andere verbinding</button></>}</div></header>
      {!token && <form className="work-connect" onSubmit={event => { event.preventDefault(); void connect(); }}><label htmlFor="memory-token">Leon-dashboardtoken</label><input id="memory-token" type="password" autoComplete="off" value={draftToken} onChange={event => setDraftToken(event.target.value)} required/><button className="pause-control" type="submit"><LockKeyhole size={15}/> Verbind</button><p>Het token blijft alleen in het geheugen van deze weergave.</p></form>}
