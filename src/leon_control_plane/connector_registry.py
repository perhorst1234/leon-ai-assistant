...
DEFAULT_CONNECTOR_MANIFESTS: list[dict[str, Any]] = [
    {
        "connector_id": "weather-open-meteo",
        "name": "Amsterdam weather via Open-Meteo",
        "connector_type": "browser_research",
        "status": "approved_readonly",
        "purpose": "Fixed-location current conditions and three-day forecast for the private Today dashboard.",
        "read_scopes": ["weather:current_read"],
        "write_scopes": [],
        "external_system": True,
        "external_effects": ["weather_api_read"],
        "required_env_keys": [],
        "approval_required_for": ["write", "external_write", "connect"],
        "allowed_without_approval": ["read"],
        "forbidden_actions": ["read_raw_secret", "read_secret_value", "location_override", "external_write"],
        "notes": "No account or API key. Fixed Amsterdam coordinates, bounded response, ten-minute cache and CC BY 4.0 attribution.",
    },
    {
    ...
    {
        "connector_id": "firecrawl-research",
        "name": "Firecrawl search-only research",
        "connector_type": "browser_research",
        "status": "disabled",
        "purpose": "Bounded read-only web search with source metadata; scrape, crawl and extraction are out of scope.",
        "read_scopes": ["web:search_only"],
        "write_scopes": ["web:scrape", "web:crawl"],
        "external_system": True,
        "external_effects": ["web_requests"],
        "required_env_keys": ["FIRECRAWL_API_KEY"],
        "approval_required_for": ["write", "external_write", "connect"],
        "allowed_without_approval": ["read"],
        "forbidden_actions": ["read_raw_secret", "read_secret_value", "secret_exfiltration", "scrape", "crawl", "extract"],
        "rollback_notes": "Disable connector and revoke the Firecrawl key; preserve only redacted audit evidence.",
        "notes": "Search-only v2 API contract. Explicitly disabled until enabled in local configuration.",
    },
    {
        "connector_id": "mail",
        "name": "Mail",
