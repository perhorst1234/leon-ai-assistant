...
SOURCE_MANIFEST_SHA256 = "9c6ed985e94a0c6bb9885ec3aae964a4d4404d103f8832f24293301940667b96"
SOURCE_ARCHIVE_SHA256 = "3ba53ffeb29f5af4477ad749c14cb627291c7ad64e9c294157dd5dea2b9abfcd"
UPSTREAM_NAME = "fetchaller-mcp"
UPSTREAM_VERSION = "3.5.4"
SOURCE_LICENSE = "MIT"
SOURCE_LICENSE_SHA256 = "584263df63d4217a82ba7ba80bda238020294f30767ed8efac35bec5b55fddf6"
EXPECTED_TOOLS = ("search_aliexpress", "get_aliexpress_product", "search_aliexpress_bundle_deals", "build_aliexpress_bundle")
MANIFEST_NAME = ".leon-install-manifest.json"
MAX_MANIFEST_BYTES = 8 * 1024 * 1024
ROOT = Path(__file__).resolve().parents[2]
SOURCE = ROOT / "integrations" / "shopper" / "aliexpress-safe"
TARGET = ROOT / ".runtime" / "mcp" / f"aliexpress-safe-{VERSION}"
...
def _assert_local_target(target: Path) -> None:
    ...
    try:
        ...
        raise ValueError("AliExpress runtime target must stay inside repository") from exc
    if not relative.parts:
        raise ValueError("AliExpress runtime target must stay inside repository")
    current = root
    for part in relative.parts[:-1]:
        current /= part
        if current.is_symlink():
            raise ValueError("AliExpress runtime parent may not be a symlink")
    ...
    for base, directories, filenames in os.walk(root, followlinks=False):
        ...
        for name in directories:
            if (base_path / name).is_symlink():
                raise RuntimeError("AliExpress artifact contains a symlink")
        for name in filenames:
            path = base_path / name
            if path.name == exclude:
                continue
            if path.is_symlink() or not path.is_file():
                raise RuntimeError("AliExpress artifact contains an unsafe file")
            result[path.relative_to(root).as_posix()] = _hash_file(path)
    if not result:
        raise RuntimeError("AliExpress artifact contains no files")
...
def _verify_source() -> None:
    ...
    if not manifest_path.is_file() or manifest_path.is_symlink() or manifest_path.stat().st_size > MAX_MANIFEST_BYTES:
        raise RuntimeError("AliExpress source manifest is missing or unsafe")
    raw = manifest_path.read_bytes()
    if not hmac.compare_digest(hashlib.sha256(raw).hexdigest(), SOURCE_MANIFEST_SHA256):
        raise RuntimeError("AliExpress source manifest integrity check failed")
    document = json.loads(raw)
    if (
        not isinstance(document, dict)
        or set(document) != {"files", "name", "source_archive_sha256", "source_license", "source_license_sha256", "upstream_name", "upstream_version", "version", "version_string"}
        or document["version"] != 1
        or document["name"] != PACKAGE
        or document["version_string"] != VERSION
        or document["upstream_name"] != UPSTREAM_NAME
        or document["upstream_version"] != UPSTREAM_VERSION
        or document["source_archive_sha256"] != SOURCE_ARCHIVE_SHA256
        or document["source_license"] != SOURCE_LICENSE
        or document["source_license_sha256"] != SOURCE_LICENSE_SHA256
        or not isinstance(document["files"], dict)
    ):
        raise RuntimeError("AliExpress source manifest schema is invalid")
    ...
    if not isinstance(expected, dict) or set(current) != set(expected) or not all(
        ...
        raise RuntimeError("AliExpress source file integrity check failed")
...
def _verify_package_files(root: Path) -> None:
    ...
    if (
        ...
        raise RuntimeError("AliExpress package lock contract differs from approved source")
    runtime = root / "dist" / "index.js"
    if not runtime.is_file() or runtime.is_symlink():
        raise RuntimeError("AliExpress runtime entrypoint is missing or unsafe")
...
def _verify_runtime_manifest(target: Path) -> None:
    ...
    if not path.is_file() or path.is_symlink() or path.stat().st_size > MAX_MANIFEST_BYTES:
        raise RuntimeError("AliExpress runtime manifest is missing or unsafe")
    ...
    if (
        ...
        raise RuntimeError("AliExpress runtime manifest schema is invalid")
    ...
    if set(current) != set(expected) or not all(
        ...
        raise RuntimeError("AliExpress runtime file integrity check failed")


def verify_installation(target: Path = TARGET) -> dict[str, object]:
    _assert_local_target(target)
    if not target.is_dir() or target.is_symlink():
        raise RuntimeError("AliExpress runtime is missing or unsafe")
...
def install(*, target: Path = TARGET, npm: str = "npm") -> dict[str, object]:
    ...
    if not executable:
        raise RuntimeError("npm is required to stage AliExpress MCP")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage or verify disabled AliExpress MCP runtime")
